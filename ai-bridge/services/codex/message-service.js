 ...
 * Codex 在只执行工具或命令的场景下可能没有自然语言收尾，这里按事件状态生成稳定摘要，
 * 避免聊天区只展示工具块而缺少“本轮已经结束”的用户可见信号。
 *
 * @param {ReturnType<typeof createInitialEventState>} state Codex 事件流状态
 * @return {string} 可展示给用户的任务完成摘要
 */
 ...
 * 这里复用现有任务通知块协议，保持前端渲染样式一致，并把最终响应同步写回 state，
 * 便于 Java 通知链路和日志读取本轮最终展示内容。
 *
 * @param {ReturnType<typeof createInitialEventState>} state Codex 事件流状态
 * @param {(msg: object) => void} emitMessage 统一消息输出函数
 * @return {string} 已输出的任务完成摘要
 */
